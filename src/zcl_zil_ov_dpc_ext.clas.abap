...
CLASS ZCL_ZIL_OV_DPC_EXT IMPLEMENTATION.
  ...
  METHOD ovcabset_get_entity.
    DATA: ld_ordemid TYPE ziltb_ovcab-ordemid.
    ...
    ENDIF.
      ...
      IF sy-subrc = 0.
        ...
        CONVERT DATE ls_cab-data_criacao
                TIME ls_cab-data_hora
                INTO TIME STAMP er_entity-datacriacao
                TIME ZONE sy-zonlo.
      ELSE.
  ...
  method OVCABSET_GET_ENTITYSET.
    Data: lt_cab TYPE STANDARD TABLE OF ZILTB_OVCAB.
    DATA: ls_cab TYPE ZILTB_OVCAB.
  ...
  METHOD ovitemset_get_entity.
    ...
    IF sy-subrc <> 0.
      ...
      lo_msg->add_message_text_only(
        EXPORTING
          iv_msg_type = 'E'
          iv_msg_text = 'Id da Orden não informado'
       ).
    ENDIF.
